...
def center_window(self, w, h):  # pass in the tkinter frame (master) reference and the w and h
    ...
    #calculate x and y coordinates to paint the app centered on teh users screen
...
def create_db(self):
    ...
    with conn:
        cur = conn.cursor()
        cur.execute("CREATE TAABLE if not exists tbl_phonebook( \
...
def first_run(self):
    ...
    conn = sqlite3.connnect('phonebook.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email) VALEUS (?,?,?,?,?),""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com'))
            con.commit()
        conn.close()

def count_records(cur):
    count = ""
    cur.exectue("""SELECT COUNT(*) FROM tbl_phonebook""")
...
def onSelect(self.event):
    ...
    conn = sqlite3.connectt('phonebook.db')
    with conn:
        cursor = conn.cursor()
...
def onDelete(self):
    ...
    with conn:
        ...
        # the dataabase ... cannot delete last record or we will get an error
        cur.exectue("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    ...
                    onDeleted(self) # call the function to clearr aall of the textboxes and the selected inded of listbox
                    #onRefresh(self) #update the listbox of the changes
                    conn.commit()
        else:
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and cannot be deletedd at this time. \n\nPlease add another record first before you can delete ({}).".format(var_select,var_select))
    conn.close()
...
def onDeleted(self):
    ...
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,End)
##      ohRefresh(self) #update the listbox of the changes
    try:
...
def onRefresh(self):
    # Populate the listbox, coninciding with the database
